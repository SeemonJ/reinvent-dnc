...
class Memory(nn.Module):
    ...
    def __init__(self, input_size, device, mem_size=512, cell_size=32, read_heads=4, independent_linears=True):
    ...
    def reset(self, batch_size=1, hidden=None, erase=True):
        ...
        if hidden is None:
            return {
                'memory': T.zeros(b, m, w).fill_(0).to(self.device),
                'link_matrix': T.zeros(b, 1, m, m).to(self.device),
                'precedence': T.zeros(b, 1, m).to(self.device),
                'read_weights': T.zeros(b, r, m).fill_(0).to(self.device),
                'write_weights': T.zeros(b, 1, m).fill_(0).to(self.device),
                'usage_vector': T.zeros(b, m).to(self.device)
            }
        else:
    ...
    def allocate(self, usage, write_gate):
        ...
        v = sorted_usage.data.new(batch_size, 1).fill_(1).to(self.device)
    ...
    def write(self, write_key, write_vector, erase_vector, free_gates, read_strengths, write_strength, write_gate,
        ...
        # get current usage
        hidden['usage_vector'] = T.clamp(self.get_usage_vector(
            hidden['usage_vector'],
            free_gates,
            hidden['read_weights'],
        ...
        hidden['write_weights'] = T.clamp(self.write_weighting(
            hidden['memory'],
