...
from reinforcement.reinforcement import reinforcement_learning
from reinforcement import scaffold, scoring
from utils.utils import format_help_for_epilog, UnderscoreArgumentParser, FilePath, find_dir_suffix
...
def main():
    ...
    prior = mm.Model.load_from_file(args.prior, sampling_mode=True)
    if args.agent == "None":
        agent = mm.Model.load_from_file(args.prior, sampling_mode=True)
    else:
        agent = mm.Model.load_from_file(args.agent, sampling_mode=True)
    ...
    reinforcement_learning(agent=agent, prior=prior, scoring_function=scoring_function, scaffoldfilter=scaffoldfilter,
                           n_steps=args.steps, experience_replay=args.experience, reset=args.reset,
                           reset_score_cutoff=args.reset_cutoff_score, logdir=args.logdir, resultdir=args.resultdir,
                           lr=args.lr, sigma=args.sigma, batch_size=args.batch_size)
